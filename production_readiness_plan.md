...
- `scripts/smoke-test-3-usernames.mjs:16-22` — `Ziad/FleetOps123`
- `scripts/smoke-test-web.mjs:175` — same
- `scripts/seed-vessel-from-shore.mjs:9` — `abdallah/asm12345`, `zyad/abm12345`
- Real-looking emails: `zeyad_yasser2010@yahoo.com`, `zyad@abmaritime.com.jo`, `abdallah@asm.com.jo`
...
3. Add a CI grep guard at the lint step:
   ```bash
   if grep -rn "FleetOps123\|asm12345\|abm12345" scripts/ apps/ packages/; then
...
**Verify:** the grep guard runs in CI; `git log -p --all | grep FleetOps123` returns nothing.
