...
beforeAll(async () => {
  ...
  await prisma.withTenant(tenantId, async (tx) => {
    await tx.vessel.create({ data: { id: vesselId, tenantId, name: 'MV Recorder' } });
  });
});
...
describe('OutboxRecorder — shore', () => {
  it('recordUpsert appends an outbox row + sync_records inside the caller tx', async () => {
    ...
    const outboxRow = await prisma.withTenant(tenantId, (tx) =>
      tx.outbox.findFirst({ where: { tenantId, vesselId, entityType: 'Component', entityId } }),
    );
  ...
  it('mints monotonically increasing HLCs for the same (tenant, vessel)', async () => {
    ...
    const hlcs: string[] = [];
    for (let i = 0; i < 5; i++) {
      const r = await prisma.withTenant(tenantId, async (tx) =>
  ...
  it('recordDelete writes a delete outbox row and tombstones sync_records', async () => {
    ...
    const { outboxRows, syncRow } = await prisma.withTenant(tenantId, async (tx) => ({
      outboxRows: await tx.outbox.findMany({
  ...
  it('different (tenant, vessel) pairs use different HLC clocks', async () => {
    ...
    await prisma.withTenant(tenantId, (tx) =>
      tx.vessel.create({ data: { id: otherVesselId, tenantId, name: 'MV Other' } }),
    );
    try {
