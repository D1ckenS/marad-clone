...
beforeAll(async () => {
  ...
  await prisma.withTenant(tenantId, async (tx) => {
    await tx.vessel.create({ data: { id: vesselId, tenantId, name: 'MV Schema' } });
...
describe('P1-1 maintenance schema — Postgres', () => {
  it('round-trips a hierarchical Component with a master link', async () => {
    ...
    await prisma.withTenant(tenantId, async (tx) => {
      await tx.masterComponent.create({
  ...
  it('rejects a Job whose interval columns are both null (CHECK constraint)', async () => {
    ...
    await prisma.withTenant(tenantId, (tx) =>
      tx.component.create({ data: { id: componentId, tenantId, vesselId, name: 'C-no-interval' } }),
    );
  ...
  it('schedules a JobInstance and signs it off via JobHistory', async () => {
    ...
    await prisma.withTenant(tenantId, async (tx) => {
      await tx.component.create({ data: { id: componentId, tenantId, vesselId, name: 'Pump' } });
      await tx.job.create({
        data: {
          id: jobId,
          tenantId,
    ...
    const stored = await prisma.withTenant(tenantId, (tx) =>
      tx.jobHistory.findUnique({ where: { id: historyId } }),
    );
  ...
  it('blocks UPDATE of business columns on JobHistory (immutability trigger)', async () => {
    ...
    await expect(
      prisma.withTenant(tenantId, (tx) =>
        tx.jobHistory.update({ where: { id: historyId }, data: { notes: 'tampered' } }),
      ),
    ).rejects.toThrow(/immutable/);

    // Sync-meta change must still succeed (soft-delete path).
    const softDeleted = await prisma.withTenant(tenantId, (tx) =>
      tx.jobHistory.update({
        where: { id: historyId },
  ...
  it('records a RunningHourReading', async () => {
    ...
    await prisma.withTenant(tenantId, async (tx) => {
      ...
      await tx.runningHourReading.create({
        data: {
          id: ulid(),
          tenantId,
          vesselId,
    ...
    const readings = await prisma.withTenant(tenantId, (tx) =>
      tx.runningHourReading.findMany({ where: { tenantId, vesselId, componentId } }),
    );
