...
beforeAll(async () => {
  ...
  const adminLogin = await request(app.getHttpServer())
    ...
    .send({ tenantId: ctx.tenantId, identifier: 'admin@safety-vessel.test', password: 'AdminP@ss1' });
  ...
  const chiefLogin = await request(app.getHttpServer())
    .post('/api/v1/auth/login')
    .send({ tenantId: ctx.tenantId, identifier: 'chief@safety-vessel.test', password: 'TestP@ss!1' });
