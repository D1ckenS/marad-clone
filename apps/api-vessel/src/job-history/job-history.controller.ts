...
export class JobHistoryController {
  ...
  /**
   * Sign off a JobInstance on the vessel: creates the immutable JobHistory
   * row + flips the parent instance to DONE. Photos are multipart files
   * under the field name `photos`; metadata arrives as form fields.
   */
  @Post('job-instances/:id/sign-off')
  @UseInterceptors(
    FilesInterceptor('photos', MAX_PHOTOS, { limits: { fileSize: MAX_PHOTO_BYTES } }),
  )
  signOff(
