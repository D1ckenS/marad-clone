...
export class QhseDocumentController {
  ...
  @Post() create(@AuthCtx() auth: AuthContext, @Body() dto: CreateQhseDocumentDto) {
  ...
  @Get(':id/revisions') getRevisions(@AuthCtx() auth: AuthContext, @Param('id') id: string) {
    return this.svc.getRevisions(auth, id);
  }
  @Post('revisions/:revisionId/approve') approveRevision(
    @AuthCtx() auth: AuthContext,
