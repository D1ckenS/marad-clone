import { Injectable, NotFoundException } from '@nestjs/common';
import { and, desc, eq, isNull } from 'drizzle-orm';
import { newId } from '@fleetops/domain';
import type { AuthContext } from '../auth/auth-context';
import { requireVesselId } from '../auth/vessel-bound';
import { DrizzleService } from '../db/drizzle.service';
import { rotations } from '../db/schema';
import { OutboxRecorder } from '../sync/outbox-recorder';
import type { CreateRotationDto, UpdateRotationDto } from './dto/create-rotation.dto';

const ENTITY = 'Rotation';

@Injectable()
export class RotationService {
  constructor(
    private readonly drizzle: DrizzleService,
    private readonly recorder: OutboxRecorder,
  ) {}

  create(auth: AuthContext, dto: CreateRotationDto) {
    const vesselId = dto.vesselId ?? requireVesselId(auth);
    const id = newId();
    const nowIso = new Date().toISOString();
    return this.drizzle.db.transaction((tx) => {
      const fields = {
        vesselId,
        crewMemberId: dto.crewMemberId,
        plannedSignOn: dto.plannedSignOn,
        plannedSignOff: dto.plannedSignOff,
        notes: dto.notes ?? null,
      };
      const { hlc } = this.recorder.recordUpsert(
        tx,
        { tenantId: auth.tenantId, vesselId },
        ENTITY,
        id,
        fields,
      );
      const [row] = tx
        .insert(rotations)
        .values({
          id,
          tenantId: auth.tenantId,
          ...fields,
          createdAt: nowIso,
          updatedAt: nowIso,
          hlc,
        })
        .returning()
        .all();
      return row;
    });
  }

  findAll(auth: AuthContext, query: { vesselId?: string; crewMemberId?: string; status?: string }) {
    const filters = [eq(rotations.tenantId, auth.tenantId), isNull(rotations.deletedAt)];
    if (query.vesselId) filters.push(eq(rotations.vesselId, query.vesselId));
    if (query.crewMemberId) filters.push(eq(rotations.crewMemberId, query.crewMemberId));
    if (query.status) filters.push(eq(rotations.status, query.status as never));
    return this.drizzle.db
      .select()
      .from(rotations)
      .where(and(...filters))
      .orderBy(desc(rotations.plannedSignOn))
      .all();
  }

  findOne(auth: AuthContext, id: string) {
    const row = this.drizzle.db
      .select()
      .from(rotations)
      .where(
        and(
          eq(rotations.id, id),
          eq(rotations.tenantId, auth.tenantId),
          isNull(rotations.deletedAt),
        ),
      )
      .get();
    if (!row) throw new NotFoundException(`Rotation ${id} not found`);
    return row;
  }

  update(auth: AuthContext, id: string, dto: UpdateRotationDto) {
    const existing = this.findOne(auth, id);
    const fields: Record<string, unknown> = {};
    if (dto.status !== undefined) fields['status'] = dto.status;
    if (dto.plannedSignOn !== undefined) fields['plannedSignOn'] = dto.plannedSignOn;
    if (dto.plannedSignOff !== undefined) fields['plannedSignOff'] = dto.plannedSignOff;
    if (dto.actualSignOn !== undefined) fields['actualSignOn'] = dto.actualSignOn ?? null;
    if (dto.actualSignOff !== undefined) fields['actualSignOff'] = dto.actualSignOff ?? null;
    if (dto.notes !== undefined) fields['notes'] = dto.notes;
    return this.drizzle.db.transaction((tx) => {
      const { hlc } = this.recorder.recordUpsert(
        tx,
        { tenantId: auth.tenantId, vesselId: existing.vesselId },
        ENTITY,
        id,
        fields,
      );
      const [row] = tx
        .update(rotations)
        .set({ ...fields, updatedAt: new Date().toISOString(), hlc } as never)
        .where(eq(rotations.id, id))
        .returning()
        .all();
      return row;
    });
  }

  softDelete(auth: AuthContext, id: string) {
    const existing = this.findOne(auth, id);
    this.drizzle.db.transaction((tx) => {
      this.recorder.recordDelete(
        tx,
        { tenantId: auth.tenantId, vesselId: existing.vesselId },
        ENTITY,
        id,
      );
      tx.update(rotations)
        .set({ deletedAt: new Date().toISOString(), updatedAt: new Date().toISOString() })
        .where(eq(rotations.id, id))
        .run();
    });
  }
}
