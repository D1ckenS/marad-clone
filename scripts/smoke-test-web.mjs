...
// ── tenant admin pass ────────────────────────────────────────────────────────
{
  ...
  console.log('=== Tenant admin pass (zyad / REDACTED) ===');
  await loginAs(page, { identifier: 'zyad', password: 'REDACTED' });
...
// ── super admin pass ─────────────────────────────────────────────────────────
{
  const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const page = await ctx.newPage();
  console.log('\n=== Super admin pass (Ziad / REDACTED) ===');
  await loginAs(page, { identifier: 'Ziad', password: 'REDACTED' });
